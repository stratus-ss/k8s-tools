...
class TestValidator:
    ...
    def validate_resource_structure(
        ...
        assert (
            extracted["apiVersion"] == expected_api_version
        ), f"Expected apiVersion {expected_api_version}, got {extracted.get('apiVersion')}"
        assert extracted["kind"] == expected_kind, f"Expected kind {expected_kind}, got {extracted.get('kind')}"
        assert (
            extracted["metadata"]["name"] == expected_name
        ), f"Expected name {expected_name}, got {extracted['metadata'].get('name')}"
        assert (
            extracted["metadata"]["namespace"] == expected_namespace
        ), f"Expected namespace {expected_namespace}, got {extracted['metadata'].get('namespace')}"
    ...
    def validate_bmh_essentials(self, extracted: Dict[str, Any], original_data: Dict[str, Any]) -> None:
        ...
        assert (
            bmc_config["credentialsName"] == original_bmc["credentialsName"]
        ), "BMC credentials name must be preserved"
        assert (
            extracted["spec"]["bootMACAddress"] == original_data["spec"]["bootMACAddress"]
        ), "Boot MAC address must be preserved"
        ...
        if "rootDeviceHints" in original_data["spec"]:
            assert (
                extracted["spec"]["rootDeviceHints"]["deviceName"]
                == original_data["spec"]["rootDeviceHints"]["deviceName"]
            ), "Root device hints must be preserved"
    ...
    def validate_machine_essentials(self, extracted: Dict[str, Any], original_data: Dict[str, Any]) -> None:
        ...
        assert (
            provider_value["kind"] == "BareMetalMachineProviderSpec"
        ), "Provider spec must be BareMetalMachineProviderSpec"
    ...
    def validate_machine_provider_spec(self, extracted: Dict[str, Any], original_data: Dict[str, Any]) -> None:
        ...
        assert (
            provider_value["apiVersion"] == original_provider["apiVersion"]
        ), "Provider spec apiVersion must be preserved"
        if "customDeploy" in original_provider:
            assert (
                provider_value["customDeploy"]["method"] == original_provider["customDeploy"]["method"]
            ), "Custom deploy method must be preserved"
        ...
        if "userData" in original_provider:
            assert (
                provider_value["userData"]["name"] == original_provider["userData"]["name"]
            ), "User data name must be preserved"

    # File Operations Validation Methods
    def validate_file_creation(
        self, result_path: str, expected_suffix: str, should_exist: bool = True, expected_content: str = None
    ) -> Path:
        ...
        if should_exist:
            ...
            assert result_path.endswith(
                expected_suffix
            ), f"Wrong suffix - expected '{expected_suffix}', got: {result_path}"
        else:
    ...
    def validate_file_mapping(
        self, result_mapping: Dict[str, str], expected_keys: set, base_directory: str = None
    ) -> None:
        """Validate file mapping structure returned by copy operations."""
        assert (
            set(result_mapping.keys()) == expected_keys
        ), f"Mapping keys mismatch - expected {expected_keys}, got {set(result_mapping.keys())}"
    ...
    def validate_yaml_backup(
        self, result_path: str, expected_path: str, expected_fields: Dict[str, Any], validate_structure: bool = True
    ) -> Dict[str, Any]:
    ...
    def validate_sanitized_yaml(
        self, result_path: str, original_data: Dict[str, Any], preserve_fields: list = None, remove_fields: list = None
    ) -> Dict[str, Any]:
    ...
    def __call__(
        ...
        return self.validate_resource_structure(
            extracted, expected_api_version, expected_kind, expected_name, expected_namespace
        )
...
def sample_secret_with_runtime_metadata(runtime_metadata_factory):
    ...
    return {
        ...
        "metadata": {
            ...
            **runtime_metadata_factory(
                uid="test-uid", manager="kubectl", owner_kind="Machine", finalizer="cleanup.finalizer"
            ),
        },
        "data": {"key": "value"},
    }
    ...
    def _create_linked_resources(
        ...
        return {
            "bmh": {
                ...
                "spec": {
                    "bmc": {"address": bmc_address, "credentialsName": bmc_secret_name},
...
def sample_control_plane_resources(linked_resource_factory):
    ...
    """
    return linked_resource_factory(node_name="control-node", machine_role="master", user_data_name="master-user-data")
...
def sample_namespace_consistent_resources(linked_resource_factory):
    ...
    """
    return linked_resource_factory(node_name="test-bmh", namespace="openshift-machine-api", user_data_name="user-data")
...
@pytest.fixture(params=list(NETWORK_SCENARIO_CONFIGS.keys()), ids=lambda scenario_id: scenario_id)
...
def mock_backup_manager_factory(mock_printer, mock_execute_oc_command):
    ...
    def create_mock_manager(
        execute_oc_return_value: Any = None, cluster_name: str = None, backup_dir: str = None
    ) -> BackupManager:
...
def machine_factory():
    ...
    def _create_machine(
        ...
        """
        labels = {"machine.openshift.io/cluster-api-machine-role": machine_role}
        ...
        provider_spec_value = {"apiVersion": "machine.openshift.io/v1beta1", "kind": "BareMetalMachineProviderSpec"}
        ...
        machine = {
            "apiVersion": "machine.openshift.io/v1beta1",
            "kind": "Machine",
            "metadata": {"name": machine_name, "namespace": "openshift-machine-api", "labels": labels},
            "spec": {"lifecycleHooks": {}, "providerSpec": {"value": provider_spec_value}},
        }
        ...
        if include_full_provider_spec:
            ...
            machine["spec"]["providerSpec"]["value"].update(
                {"customDeploy": {"method": "install_coreos"}, "image": {"url": "https://example.com/rhcos.iso"}}
            )
...
def secret_factory():
    ...
    def _create_secret(
        ...
        secret = {"apiVersion": "v1", "kind": "Secret", "metadata": metadata, "type": secret_type}
        ...
        if data_keys:
            ...
            secret["data"] = {key: base64.b64encode(value.encode()).decode() for key, value in string_data.items()}
        else:
...
def nmstate_factory():
    ...
    def _create_nmstate(
        ...
        interface_config = {"name": interface_name, "type": interface_type, "state": "up" if enabled else "down"}
        ...
        if ip_address:
            ...
            interface_config["ipv4"] = {
                "enabled": True,
                "address": [{"ip": ip_address, "prefix-length": prefix_length}],
            }
        else:
            # DHCP configuration (default)
            interface_config["ipv4"] = {"enabled": enabled, "dhcp": True}

        # Build complete nmstate structure
        nmstate_config = {"interfaces": [interface_config]}
...
def pod_factory():
    ...
    def _create_pod(
        ...
        pod = {"apiVersion": "v1", "kind": "Pod", "metadata": metadata, "status": {"phase": status_phase}}
        ...
        if status_phase != "Running" or ready_condition != "True":
            pod["status"]["conditions"] = [{"type": "Ready", "status": ready_condition}]
...
def sample_etcd_control_plane_nodes(node_factory) -> Dict[str, Any]:
    ...
    """
    node1 = node_factory(node_name="ocp-control1.two.ocp4.example.com", is_control_plane=True)
    node2 = node_factory(node_name="ocp-control2.two.ocp4.example.com", is_control_plane=True)
    node3 = node_factory(node_name="ocp-control3.two.ocp4.example.com", is_control_plane=True)

    return {"apiVersion": "v1", "kind": "List", "items": [node1, node2, node3]}
...
def sample_etcd_secrets(secret_factory) -> Dict[str, Any]:
    ...
    """
    secret1 = secret_factory(secret_name="etcd-serving-ocp-control1.two.ocp4.example.com", namespace="openshift-etcd")
    secret2 = secret_factory(secret_name="etcd-serving-ocp-control3.two.ocp4.example.com", namespace="openshift-etcd")
    secret3 = secret_factory(secret_name="etcd-peer-ocp-control2.two.ocp4.example.com", namespace="openshift-etcd")
    secret4 = secret_factory(
        secret_name="etcd-serving-metrics-ocp-control2.two.ocp4.example.com", namespace="openshift-etcd"
    )

    return {"apiVersion": "v1", "kind": "List", "items": [secret1, secret2, secret3, secret4]}
...
def sample_etcd_pods_data(pod_factory) -> Dict[str, Any]:
    ...
    return {"apiVersion": "v1", "kind": "List", "items": [pod1, pod2, pod3]}
