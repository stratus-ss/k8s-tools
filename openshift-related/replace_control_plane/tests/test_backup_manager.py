...
class TestDataExtractionAndAccuracy:
    ...
    def test_bmh_extraction_comprehensive_validation(
        self, backup_manager, sample_bmh_data, resource_validator, bmh_validator
    ):
    ...
    def test_bmh_network_configuration_preservation(self, backup_manager, bmh_factory):
    ...
    def test_machine_extraction_provider_spec_preservation(self, backup_manager, machine_factory):
        """Test that Machine extraction preserves provider spec needed for bare metal deployment"""
        sample_machine_with_provider_spec = machine_factory(
            include_full_provider_spec=True, include_cluster_labels=False
        )
    ...
    def test_machine_extraction_master_node_configuration(self, backup_manager, machine_factory):
        """Test that Machine extraction properly handles master node specific configuration"""
        sample_master_machine_data = machine_factory(
            machine_name="master-machine", user_data_name="master-user-data-managed"
        )
        ...
        assert (
            extracted["spec"]["providerSpec"]["value"]["userData"]["name"]
            == sample_master_machine_data["spec"]["providerSpec"]["value"]["userData"]["name"]
        )

    def test_machine_extraction_comprehensive_validation(
        self, backup_manager, sample_machine_data, resource_validator, machine_validator
    ):
...
class TestNetworkConfigurationPreservation:
    ...
    def test_network_config_backup_preserves_configuration_data(self, backup_manager, network_config_scenarios: dict):
        ...
        result_path = backup_manager.backup_secret(
            scenario["node_name"], scenario["secret_name"], "_network-config.yaml", scenario["description"]
        )
    ...
    def test_bmh_network_data_name_linking(self, backup_manager, bmh_factory):
        ...
        sample_bmh_network_data_linking = bmh_factory(
            node_name="network-node", network_config_name="network-node-network-config-secret", include_user_data=False
        )
        ...
        assert (
            extracted["spec"]["preprovisioningNetworkDataName"]
            == sample_bmh_network_data_linking["spec"]["preprovisioningNetworkDataName"]
        )
...
class TestCrossResourceDependencyValidation:
    ...
    def test_machine_bmh_linking_validation(self, backup_manager):
        ...
        bmh_config = {
            ...
            "spec": {"userData": {"name": "master-user-data"}},  # Must match Machine
        }
    ...
    def test_user_data_consistency_validation(self, backup_manager, linked_resource_factory):
        ...
        """
        user_data_configs = [("master-user-data-managed", "master"), ("worker-user-data", "worker")]
        ...
        for user_data_name, role in user_data_configs:
            ...
            resources = linked_resource_factory(
                node_name=f"{role}-node", user_data_name=user_data_name, machine_role=role
            )
...
class TestRealBackupOperations:
    ...
    def test_backup_machine_definition_creates_valid_yaml(
        self, backup_manager, sample_machine_data, yaml_file_validator
    ):
    ...
    def test_backup_secret_creates_sanitized_yaml(self, backup_manager, sample_bmc_secret_data, yaml_file_validator):
        ...
        result_path = backup_manager.backup_secret(node_name, secret_suffix, backup_filename_suffix, secret_description)
    ...
    def test_backup_operations_use_correct_filenames(
        self, temp_backup_manager, sample_bmh_data, sample_machine_data, file_operation_validator
    ):
    ...
    def test_backup_template_bmh_comprehensive_scenarios(self, backup_manager, sample_bmh_list, sample_bmh_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            template_file_2, is_worker_2 = backup_manager.backup_template_bmh(failed_control_node=failed_node)
...
class TestRealFileCopyOperations:
    ...
    def test_copy_files_for_replacement_successful_operation(self, temp_backup_manager, file_operation_validator):
        ...
        result = temp_backup_manager.copy_files_for_replacement(bad_node, bmh_name, bad_machine, replacement_node)
    ...
    def test_copy_files_preserves_content_for_file_types(
        ...
        result = temp_backup_manager.copy_files_for_replacement(bad_node, bmh_name, machine_name, replacement_node)
