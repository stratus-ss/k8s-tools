...
class TestIntegrationWorkflows:
    ...
    def mock_external_dependencies(self):
        ...
        mock_backup_manager.backup_template_bmh.return_value = (
            "/tmp/test-backup/template.yaml",
            True,
        )  # Returns (file_path, is_worker_template)
    ...
    def test_case_1_worker_addition_full_integration(self, mock_external_dependencies):
        ...
        orchestrator = NodeOperationOrchestrator(printer=printer, **mock_external_dependencies)

        args = self.create_mock_args(add_new_node=True, replacement_node_role="worker")
    ...
    def test_case_2_control_plane_replacement_full_integration(self, mock_external_dependencies):
        ...
        args = self.create_mock_args(add_new_node=False, expand_control_plane=False, replacement_node_role="master")
    ...
    def test_case_4_control_plane_expansion_full_integration(self, mock_external_dependencies):
        ...
        args = self.create_mock_args(add_new_node=False, expand_control_plane=True, replacement_node_role="master")
    ...
    def test_workflow_step_counting_accuracy(self, mock_time, mock_external_dependencies):
        ...
        for args, expected_steps, description in test_cases:
            ...
            if step_calls:
                ...
                assert (
                    total_steps_used == expected_steps
                ), f"{description} should use {expected_steps} steps, got {total_steps_used}"
    ...
    def test_error_handling_integration(self, mock_external_dependencies):
        ...
        mock_external_dependencies["BackupManager"].return_value.setup_backup_directory.side_effect = Exception(
            "Backup failed"
        )
