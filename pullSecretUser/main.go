...
func main() {
	...
	dataType := flag.String("data-type", "dockerconfigjson", "The heading of the in the 'data' section of the secret you wish to inspect")
	flag.Parse()
	...
	for _, projectInfo := range namespaces.Items {
		...
		for _, secretsInfo := range all_secrets.Items {
			...
			for secretsKey, secretValue := range individual_secret.Data {
				if strings.Contains(secretsKey, *dataType) {
